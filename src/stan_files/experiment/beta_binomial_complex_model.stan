...
parameters {
  ...
  matrix <lower = 0> [N_gene, N_sample] phi;
  // vector <lower = 0> [N_gene] phi;
  real<lower = 0> tau;
}
...
transformed parameters {
  ...
  // vector <lower = 0> [N_gene] phi [N_sample];
  ...
  for(i in 1:N_sample) {
    a[i] = inv_logit(alpha_gene + beta[i]*X[i]) .* phi[,i];
    b[i] = phi[,i] - a[i];
  }
}

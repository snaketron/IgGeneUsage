...
parameters {
  ...
  // noise parameters
  vector <lower = 0, upper = 1> [N_gene] p_mu;
  real <lower = 0> p_phi;
  real <lower = 0> p_tau;
...
transformed parameters {
  ...
  vector <lower = 0> [N_gene] p_a [N_sample];
  vector <lower = 0> [N_gene] p_b [N_sample];
...
model {
  for(i in 1:N_sample) {
    target += log_mix(z,
                      ...
                      beta_binomial_lpmf(Y[,i] | N[i], p_a[i], p_b[i]));
  }
  ...
  for(j in 1:N_gene) {
    for(i in 1:N_sample) {
      ...
      log_lik[j,i] = log_mix(z,
                        ...
                        beta_binomial_lpmf(Y[j,i] | N[i], p_a[i][j], p_b[i][j]));

      zprime = bernoulli_rng(z);
      if(zprime == 1) {
         Yhat[j, i] = beta_binomial_rng(N[i], a[i][j], b[i][j]);
      } else { //noise term
         Yhat[j, i] = binomial_rng(N[i], p_mu[j]);
      }

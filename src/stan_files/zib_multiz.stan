...
model {
  for(i in 1:N_sample) {
    for(j in 1:N_gene) {
      target += zero_inflated_binomial_blogit_lpmf(Y[j, i] | N[i], alpha_gene[j] + beta[i][j]*X[i], z[j]);
    }
  }
...
generated quantities {
  ...
  for(j in 1:N_gene) {
    for(i in 1:N_sample) {
      ...
      log_lik[j,i] = zero_inflated_binomial_blogit_lpmf(Y[j,i] | N[i], alpha_gene[j] + beta[i][j] * X[i], z[j]);
      Yhat[j, i] = binomial_rng(N[i], (1-z[j])*inv_logit(alpha_gene[j]+beta[i][j]*X[i]));

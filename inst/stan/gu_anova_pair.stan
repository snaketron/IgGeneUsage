...
data {
  int <lower=0> N_subject; // number of subjects (repertoires)
  ...
  int <lower=1> S [N_sample]; // subject index
}
...
parameters {
  ...
  // aux variables
  vector [N_gene] alpha_z [N_subject];
...
transformed parameters {
  ...
  for(k in 1:N_group) {
    beta_gene_mu[k] = 0 + beta_pop_sigma[k] * beta_gene_z[k];
  }
  alpha_gene_mu = alpha_pop_mu + alpha_pop_sigma * alpha_gene_z;
  for(j in 1:N_subject) {
    alpha[j] = alpha_gene_mu + alpha_gene_sigma * alpha_z[j];
  }
  for(i in 1:N_sample) {
    beta[i] = beta_gene_mu[G[i]] + beta_gene_sigma[G[i]] * beta_z[i];
    a[i] = inv_logit(alpha[S[i]] + beta[i]) * phi;
    b[i] = phi - a[i];
  }
}
...
model {
  ...
  for(i in 1:N_group) {
    target += std_normal_lpdf(beta_gene_z[i]);
  }

...
Major difference: the zero-inflation parameter $\kappa$ is allowed to vary 
from gene-to-gene.
...
\delta_{j} \sim \operatorname{Normal}\left(\hat{\delta}_{\mu}, 
\hat{\delta}_{\sigma} \right) \\
\hat{\delta}_{\mu} \sim \operatorname{Normal}\left(0.0, 5.0\right) \\
\gamma_{\sigma}, \hat{\gamma}_{\sigma}, \delta_{\sigma}, \hat{\delta}_{\sigma} 
\sim \operatorname{Cauchy^{+}}\left(0, 1\right) \\
\phi \sim \operatorname{Exponential}\left(\tau\right) \\
\tau \sim \operatorname{Gamma}\left(3, 0.1\right) \\
\kappa_{j} \sim \operatorname{Beta_proportion}\left({\kappa_\mu}, {\kappa_\phi}\right)
\kappa_{\mu} \sim \operatorname{Beta}\left({1, 5\right)
\kappa_{\phi} \sim \operatorname{Exponential}\left(0.05\right)
\end{align}
...
ggplot(data = M$ppc$ppc_condition)+
  geom_errorbar(aes(x = gene_name, ymin = ppc_L_prop*100, 
                    ymax = ppc_H_prop*100), col = "darkgray", width = 0.1)+

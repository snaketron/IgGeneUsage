...
The sum of all gene usage counts (column 4) in a given repertoire are equal to 
the total gene usage in that repertoire. The remaining IgGeneUsage inputs help 
the MCMC sampling.



# Case Study I
IgGeneUsage has built-in datasets of gene usage. The dataset IGHV_HCV used in
this case study contains publicly available data of human immunoglobulin heavy 
chain VDJ rearrangements from a study that evaluates the effect of HCV on the 
human BCR repertoire[^1]. The dataset consists of a population of class-switched 
memory (CSM) B cells of 22 HCV-infected patients (HCV\textsuperscript{+}) and 7 
healthy donors (HD). For the analysis of differential gene usage between 
(HCV\textsuperscript{+}) and HD we use only one of the two replicates of the 
CSM population. Gene usage data is available for 69 IGHV gene segments.
...
and compare them against the observed data (x-axis). If the point are near 
...
Lets also visualize the gene usage frequencies. Bubble size represents 
total usage in each repertoire.
...
analysis of differential gene usage with the T-test. Prior to using the T-test, we must convert the usage frequencies into proportions. This is automatically
done by IgGeneUsage.
...
likely cause for the disagreement. Discarding information about the sample 
...
Next, we compare the probabilities of differential gene usage ($\pi$) with the 
FDR corrected P-values (-log10 scale) from the U-test. Dashed lines show 
significancelevels of 0.05 and 0.01. The U-test finds no evidence of 
...
We use a small subset from a study evaluating vaccine-induced changes in B-cell
populations [^3]. The data is publicly provided by R-package alakazam (version
0.2.11). The IGHV family usage is reported in four B-cell populations (repertoires IgM, IgD, IgG and IgA) across two timepoints (conditions = -1 hour
and +7 days). In this case study we investigate the overal effect of the vaccine
on the IGHV family usage.
...
genes (shown as points). No genes witn $\pi > 0.95$ are shown.
...
## Comparison with the Wilcoxon signed-rank test (U-test)
No evidence of differential usage.
...
Here we focus on the two viruses InfluenzaA and CMV which represent the two 

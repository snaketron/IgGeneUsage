...
### Results
This following figure shows the main result of IgGeneUsage $\rightarrow$ 
disruption effects on each gene due to the biological condition. On the y-axis 
we show the probability of differential usage ($\pi$) in different genes shown 
as points. The names of genes with $\pi > 0.95$ are shown.
...
ggplot()+
  ...
  geom_text(data = stats[stats$pmax >= 0.95, ], 
...
#### Promising results [%]
Lets visualize the raw data of the genes with promising probability of 
differential gene usage ($\pi > 0.95$).
...
ggplot(data = stats)+
  ...
  geom_errorbarh(aes(y = gene_fac, xmin = effect_L, xmax = effect_H))+
...
ggplot(data = stats)+
  geom_vline(xintercept = 0, linetype = "dashed", col = "darkgray")+
  geom_errorbarh(aes(y = as.numeric(as.character(gene_fac)), 
                     xmin = effect_L, xmax = effect_H))+
...
p-values obtained from the t-test. Legend:
  ...
  * y-axis: FDR-corrected p-value shown on -log10 scale. Dashed lines represent 
  the significance levels of 0.05 and 0.01. 
  * x-axis: probability of differential usage
  * color: effect size excludes null. 

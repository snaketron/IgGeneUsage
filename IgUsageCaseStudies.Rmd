...
TRV_Cancer$sample_id <- TRV_Cancer$IndividualID
...
```{r, fig.width=6, fig.height=3.5}
group.ppc <- M$group.ppc$group.ppc
group.ppc <- group.ppc[group.ppc$gene_name %in% c("TRBV25-1"), ]

point.data <- M$group.ppc$individual.pct.data
point.data <- point.data[point.data$gene_name %in% c("TRBV25-1"), ]
ggplot()+
  geom_errorbar(data = group.ppc,
                aes(x = gene_name, ymin = ppc.L, 
                    ymax = ppc.H, col = condition),

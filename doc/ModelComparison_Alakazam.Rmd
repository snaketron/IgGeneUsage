---
title: "Model Comparison (Data: Alakazam)"
author: "SK"
date: "June 25, 2019"
output:
  BiocStyle::html_document
vignette: >
  %\VignetteIndexEntry{Model Comparison (Data: Alakazam)}
...
```{r}
B <- get(load(file = "R/dev/alakazam_binomial.RData"))
BB <- get(load(file = "R/dev/alakazam_beta_binomial_model.RData"))
ZIBB <- get(load(file = "R/dev/alakazam_zibb_model.RData"))
rm(M)
```
...
## Prediction of overall gene usage [%]
  * Error bars represent 95\% HDI
...
Five genes for which the pairwise models inferred most discrepant usage 
...
## Reality check
Given that zero-inflation does not play a large role in this dataset, we see no
difference between the estimates produced by ZIBB and BB. On the other hand B is
overconfident (narrow usage estimates) when there are few zeros among the other 
gene usages (IGHV1 and IGHV5).
...
# interesting genes
interesting.hits <- effect.comparsion$gene_name

# visualize
ggplot(data = viz[viz$gene_name %in% interesting.hits, ])+
  geom_point(aes(x = gene_name, y = gene_usage_pct, 
                 fill = condition, shape = condition), 
             alpha = 0.5, stroke = 0, size = 2, 
             position = position_jitterdodge(dodge.width = 0.75, 
                                             jitter.width = 0.15))+
  geom_errorbar(data = group.stats[group.stats$gene_name %in%interesting.hits,],
                aes(x = gene_name, ymin = ppc.L, ymax = ppc.H, 
                    linetype = condition, col = model),
